...
Polymorphic calculation model with subclasses:
- Addition
- Subtraction
- Multiplication
- Division
...
class AbstractCalculation:
    ...
    def create(cls, calculation_type: str, user_id: uuid.UUID, inputs: List[float]) -> "Calculation":
        """
        Factory method: returns the correct subclass instance.
        """
        calculation_classes = {
            "addition": Addition,
...
class Calculation(Base, AbstractCalculation):
    """
    Base calculation model (polymorphic root).
    """
...
class Division(Calculation):
    ...
    def get_result(self) -> float:
        ...
        for value in self.inputs[1:]:
            if value == 0:
                # Match the error message used in the operations tests
...
class Power(Calculation):
    ...
    def get_result(self) -> float:
        ...
        if len(self.inputs) != 2:
            raise ValueError(
                "Power operation requires exactly two inputs: base and exponent."
            )
        base = self.inputs[0]
